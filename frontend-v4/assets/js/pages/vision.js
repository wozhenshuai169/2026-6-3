/**
 * Vision — Image Recognition Page V2 (real file upload)
 */
(function(){
  'use strict';
  ...
  function handleFile(file){
    ...
    // Try FormData upload first, fallback to base64
    recognizeWithFormData(file);
  }

  function recognizeWithFormData(file){
    var fd=new FormData();
    fd.append('file',file);
    fd.append('roomId',roomId||'demo');
    fd.append('userId',userId||'demo');
    fd.append('currentSpotId',st.get('currentSpotId')||'');

    // Try FormData upload (backend may support multipart)
    api.upload('/vision/recognize',fd).then(function(r){
      if(r.ok&&r.data){
        document.getElementById('loading-area').classList.add('hidden');
        showResult(r.data);
      }else{
        // Fallback: try with base64 data URL from preview
    ...
    reader.onload=function(e){
      api.post('/vision/recognize',{
        ...
        if(r.ok&&r.data){
          ...
          showError((r.error&&r.error.message)||'识别失败，请确认后端已启动');
        }
      });
    };
    reader.readAsDataURL(file);
  ...
  function showResult(data){
    ...
    document.getElementById('result-description').textContent=data.description||'暂无可用的讲解内容';
    ...
    if(related.length){
      ...
      document.getElementById('result-related').classList.add('hidden');
    }
